...
//login for customer
router.post("/loginn/customer", async (req, res) => {
	try {
		console.log("Hi")
		if (req.body.email) {
...
router.post("/login/customer", async (req, res) => {
	try {
		...
		const usr = await Customer.findOne({ email }).select("email password");
		if (!usr) {
			console.log("1")
		...
		const token = await usr.generateAuthToken();
		console.log("res sent successfuly");
